...
raw_data <- read.csv(file="Building3Electricity.csv", header=TRUE, sep=",")
View(raw_data)
...
means$time <- as.character(means$time)
...
raw_data$day_mean <- apply(raw_data[,13:37], 1, mean)
...
regression_input <- data.frame(raw_data$Wday, raw_data$day_mean,
                               raw_data$night_mean)
## Rename the columns
